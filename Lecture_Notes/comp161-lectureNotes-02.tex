...
\begin{abstract}
Shotts' tutorial introduces the shell\sidenote{\url{http://linuxcommand.org/lc3_lts0010.php}} but discusses how to access it as if you're accessing the shell of the computer in front of you.  In this class, we'll be accessing the shell of a the departmental server named church\sidenote{After mathematician Alonzo Church} on the second floor of the CSB remotely using a Secure Shell\sidenote{SSH} client. We'll also briefly discuss some common patterns in shell commands, the command that you'll use to submit most of your assignments, and some strategies for moving files to and from the remote system.
\end{abstract}
